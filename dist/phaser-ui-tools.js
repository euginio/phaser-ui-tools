(function (global, factory) {
    ...
    (global = global || self, factory(global.uiWidgets = {}));
}(this, function (exports) { 'use strict';

    // Matches the alignment consts from Phaser
    var alignments = {
      TOP_LEFT: 0,
      TOP_CENTER: 1,
      TOP_RIGHT: 2,
    ...
    var Bar =
    /*#__PURE__*/
    function (_PhaserObjects$Group) {
      _inherits(Bar, _PhaserObjects$Group);
      ...
      function Bar(game) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Bar).call(this, game));
        _this.game = game;
        _this.x = x;
        _this.y = y;
    ...
    /*#__PURE__*/
    function (_Bar) {
      _inherits(DraggableBar, _Bar);

      function DraggableBar() {
        _classCallCheck(this, DraggableBar);

        return _possibleConstructorReturn(this, _getPrototypeOf(DraggableBar).apply(this, arguments));
      }
      ...
      _createClass(DraggableBar, [{
        ...
        */
        ...
        value: function horizontalTrackClick() {
          var mouseX = this.game.input.mousePointer.x; // Don't register mouse clicks on the bar itself.
        ...
        */
        ...
        value: function getMouseDelta() {
          ...
          this.mousePointer = newMousePointer; // Maximum value for the mouse position. Only update when the new position is inside the track.
        ...
        value: function scrollUp() {
          ...
          if (this.bar.y !== this.track.y && !this.barMoving) {
            ...
            var moveToY = null;
            this.barMoving = true; // Ensure the bar can't move above the track.
        ...
        value: function scrollDown() {
          if (this.bar.y + this.bar.height !== this.track.y + this.track.height && !this.barMoving) {
            ...
            var moveToY = null;
            this.barMoving = true;
            this.bar.inputEnabled = false; // Ensure the bar can't move below the track.
        ...
        value: function scrollLeft() {
          if (this.bar.x !== this.track.x && !this.barMoving) {
            ...
            var moveToX = null;
            this.barMoving = true;
            this.bar.inputEnabled = false; // Ensure the bar can't move above the track.
    ...
    function () {
      ...
      function QuantityRange(bar, startValue, maxValue) {
        _classCallCheck(this, QuantityRange);
    ...
    function () {
      ...
      function ValueRange(step, startValue, maxValue) {
        ...
        this.ratio = step / maxValue; // The ratio between the step and max can't be greater than 1.
        ...
        this.currentValue = startValue; // List of every possible step. Used for snapping into position by the ValueBar.
    ...
    function () {
      ...
      function ViewportRange(viewport, vertical) {
        _classCallCheck(this, ViewportRange);
        ...
        this.ratio = this.step / this.maxValue; // The ratio between the step and max can't be greater than 1.
        // ie: There can't be more steps than the max value.
    ...
    function (_Bar) {
      ...
      function QuantityBar(game, xy, values, vertical, reverse, trackImage, barImage, tweenParams) {
        ...
        _classCallCheck(this, QuantityBar);

        _this = _possibleConstructorReturn(this, _getPrototypeOf(QuantityBar).call(this, game, xy.x, xy.y));
        game.add.existing(_assertThisInitialized(_this));
        _this.valueRange = new QuantityRange(_assertThisInitialized(_this), values.startValue, values.maxValue);
        ...
        _this.add(_this.track); // The bar is a static image taking up the width of the track.


        _this.bar = _this.game.add.button(0, 0, _this.barImage, _this.moveContent, _assertThisInitialized(_this), 1, 0);
      ...
      _createClass(QuantityBar, [{
        ...
        value: function setMask() {
          ...
          mask.drawRect(0, 0, this.maskW, this.maskH); // mask.endFill();
        ...
        value: function create() {
          this.centerStaticAxis(); // Values for the bar's mask.

          this.maskW = this.bar.width;
          this.maskH = this.bar.height;
          this.maskX = this.bar.x;
          this.maskY = this.bar.y; // Resizes the bar.
          ...
          this.setMask(); // Determine the distance the window can scroll over

          this.windowScrollAreaSize = this.valueRange.maxValue; // Represents one fraction of the track.
        ...
        value: function adjustBar(newValue) {
          ...
          if (this.reverse) {
            if (this.vertical) {
              tween = {
                height: barSize,
                y: this.getBarFraction()
              };
            } else {
              tween = {
                width: barSize,
                x: this.getBarFraction()
              };
            }
          } else {
            if (this.vertical) {
    ...
    var Scrollbar =
    /*#__PURE__*/
    function (_DraggableBar) {
      _inherits(Scrollbar, _DraggableBar);
      ...
      function Scrollbar(game, content, draggable, vertical, trackImage, barImage, tweenParams) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Scrollbar).call(this, game));
        game.add.existing(_assertThisInitialized(_this));
        ...
        _this.trackImage = trackImage;
        _this.barImage = barImage; // The smallest pixel size allowed for the bar.
        ...
        _this.barMoving = false; // Records mouse pointer when clicking the bar.

        _this.mousePointer = null; // The track is the static area the bar will move along.
        ...
        _this.bar = _this.game.add.button(_this.x, _this.y, _this.barImage, _this.moveContent, _assertThisInitialized(_this), 1, 0);
      ...
      _createClass(Scrollbar, [{
        ...
        value: function create() {
          ...
          this.windowScrollAreaSize = this.valueRange.maxValue - this.valueRange.step; // Represents one fraction of the track.

          this.vslice = this.track.height * this.valueRange.ratio;
          this.hslice = this.track.width * this.valueRange.ratio;
          this.setTrackScrollAreaSize(); // Initial position for the bar.
        ...
        value: function setInitialBarPosition() {
          var gripPositionOnTrack = this.getBarPosition(); // Make sure the bar is physically where it should be.
        ...
        value: function getGripPositionRatio() {
          ...
          var newGripPosition = gripPositionOnTrack + mousePositionDelta; // Don't let the content scroll above or below the track's size
          ...
          var newGripPositionRatio = newGripPosition / this.trackScrollAreaSize; // If the scrollable area is less than the size of the scrollbar,
    ...
    /*#__PURE__*/
    function (_DraggableBar) {
      _inherits(ValueBar, _DraggableBar);
      ...
      function ValueBar(game, xy, values, draggable, vertical, trackImage, barImage, tweenParams) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(ValueBar).call(this, game, xy.x, xy.y));
        game.add.existing(_assertThisInitialized(_this));
        ...
        _this.add(_this.track); // If the bar is draggable, clicking the track will move the bar up or down.
        ...
        _this.bar = _this.game.add.button(_this.x, _this.y, _this.barImage, _this.moveContent, _assertThisInitialized(_this), 1, 0); // Add an invisible background.
      ...
      _createClass(ValueBar, [{
        ...
        key: "setInitialBarPosition",
        value: function setInitialBarPosition() {
          var gripPositionOnTrack = this.getBarPosition(); // The bar should always be in centered on it's current position.
    ...
    var textSprite =
    /*#__PURE__*/
    function (_PhaserObjects$Sprite) {
      _inherits(textSprite, _PhaserObjects$Sprite);
      ...
      function textSprite(game, image, label, style, x, y) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(textSprite).call(this, game, x, y, image));
        game.add.existing(_assertThisInitialized(_this));
    ...
    var textButton =
    /*#__PURE__*/
    function (_PhaserObjects$Button) {
      _inherits(textButton, _PhaserObjects$Button);
      ...
      function textButton(game, image, label, style, x, y, callback, callbackContext) {
        ...
        _this2 = _possibleConstructorReturn(this, _getPrototypeOf(textButton).call(this, game, x, y, image, callback, callbackContext));
    ...
    /*#__PURE__*/
    function (_PhaserObjects$Group) {
      _inherits(Frame, _PhaserObjects$Group);
      ...
      function Frame(game) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Frame).call(this, game));
        game.add.existing(_assertThisInitialized(_this));
        _this.x = x;
        _this.y = y;
        _this.game = game;
        _this.bg = bg; // Add background to Frame.

        if (bg !== null) {
          var bgSprite = new PhaserObjects.Sprite(game, 0, 0, bg);
          bgSprite.sendToBack();
          bgSprite.alignIn(_assertThisInitialized(_this), alignments.TOP_LEFT);
        }
    ...
    function (_Frame) {
      ...
      function Column(game) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Column).call(this, game, x, y, bg));
        _this.alignment = alignments.BOTTOM_CENTER;
        return _this;
      }
    ...
    function (_Frame) {
      ...
      function Row(game) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Row).call(this, game, x, y, bg));
        _this.alignment = alignments.RIGHT_CENTER;
        return _this;
      }
    ...
    /*#__PURE__*/
    function (_PhaserObjects$Group) {
      _inherits(Viewport, _PhaserObjects$Group);
      ...
      function Viewport(game, x, y, width, height) {
        ...
        _this = _possibleConstructorReturn(this, _getPrototypeOf(Viewport).call(this, game));
        game.add.existing(_assertThisInitialized(_this));
        _this.x = x;
        _this.y = y; // Viewport size and position, distinct from the total window size.
      ...
      _createClass(Viewport, [{
        ...
        value: function disableOutOfBounds(children, context, vertical) {
          ...
          var location;
          var contentLocation;
          var trueCoords; // Makes sure the recursive function stops when there's no children.

          if (children !== undefined) {
            for (var i = 0; i < children.length; i++) {
              child = children[i];
              child.inputEnabled = true; // An object's x/y is relative to it's parent.
    ...
    function () {
      ...
      function KeyboardGroup(game, vertical, callbackContext) {
        _classCallCheck(this, KeyboardGroup);
      ...
      _createClass(KeyboardGroup, [{
        key: "addNode",
        value: function addNode(newNode) {
          this.children.push(newNode); // Ensure the first child is already selected  when the game loads.
    ...
    function () {
      ...
      function VectorPoint() {
        ...
        _classCallCheck(this, VectorPoint);
    ...
    function () {
      ...
      function Wheel3D(game, xy, sprites, firstPlace, zoom, axis, rotations, visibleRange, tweenParams) {
        ...
        this.onForwardComplete = new Phaser.Signal(); // Group to store wheel sprites in, used for zindex sorting.
      ...
      _createClass(Wheel3D, [{
        ...
        value: function activate() {
          ...
          this.rotationAmount = 360 / this.pointsAmount; // Determines which items in the wheel should be visible.
          ...
          this.moving = false; // Stores the direction the wheel is moving in.

          this.direction = null; // Set point positions on logical circle.

          this.wheelItems = [];
          var radius = 1;
          var slice = 2 * Math.PI / this.pointsAmount; // For a vertical wheel, the X axis is -1. Points are laid out on the Y axis (height) and Z axis (depth). The wheel rotates around the X axis.
        ...
        value: function project() {
          var newTween; // Create a list with the axes, then remove the projected axis.

          var arr = ['x', 'y', 'z'];
          var idx = arr.indexOf(this.axis);
          arr.splice(idx, 1); // We only need to call all this when moving. It doesn't need to be done
          // every update

          for (var i = 0; i < this.wheelItems.length; i++) {
            // Rotate along the projected axis
            var rotationOne = this.wheelItems[i].rotate(this.axis, this.rotationAxis[this.axis]); // Rotate the other 2 axes
            ...
            transformed.lz = p.z; // Ensure active sprite has no scale/alpha changes.
            ...
            if (this.wheelItems[i].position === this.firstPlace) {
              ...
              if (this.visibleRange !== null) {
                var includes = this.visiblePositions.includes(this.wheelItems[i].position); // const includes = _.includes(
            ...
            newTween = this.game.add.tween(transformed).to({
              x: p.x,
              y: p.y
            }, this.tweenParams.duration, this.tweenParams.ease, true);
          ...
          this.group.sort('lz', Phaser.Group.SORT_ASCENDING); // Wheel's signals are dispatched by the tween's.
    ...
    exports.PhaserObjects = PhaserObjects;
